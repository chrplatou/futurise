            ...
            setTimeout(() => {
                ...
                async function getHeights(urls) {
                    ...
                    if (isNumber(highestEl) && isNumber(lowestEl)) {
                        ...
                        map.addLayer(wmsRelieffLayer);
                        raster.changed()
                        let rangeList = '<ul  style="display: flex; flex-wrap: wrap;">'
                        elevationScale.forEach(function(color,i) {
                            let height = convertRange(i, [0,elevationScale.length],[lowestEl,highestEl])
                            rangeList += `<li style="list-style: none; padding: 10px;"><span style="color: rgb(${color});"> ${color}:</span> ${height}</li>`
                        })
                        document.querySelector('#scale').innerHTML = rangeList + "</ul>";
                    }
                }
            }, 1000);
        //});
...
function getMousePos(canvas, evt) {
  ...
  canvas.addEventListener('mousemove', function(evt) {
    ...
    document.getElementById('info').innerHTML = "x: " + mousePos.x + " y: " + mousePos.y
  }, false);
var convertRange = function( value, r1, r2 ) { 
    return ( value - r1[ 0 ] ) * ( r2[ 1 ] - r2[ 0 ] ) / ( r1[ 1 ] - r1[ 0 ] ) + r2[ 0 ];
}
